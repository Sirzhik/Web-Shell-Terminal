...
Provide a secure, browser‑based terminal that lets authenticated users open interactive SSH sessions to remote hosts without leaving the web UI. The system centralises user, group and credential management, enforces authorisation, and streams the SSH session over a WebSocket so that developers, system administrators, or support personnel can work remotely from any modern browser.
