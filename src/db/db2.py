...
async def add_virtual_user(username: str, domain: str, password: str = None, ssh_key: str = None, port: int = 22) -> VirtualUsers:
    password_encrypted = ''
    ssh_key_encrypted = ''
...
# async def get_virtual_user_by_id(username: str, virtual_user_id: int) -> bool:
#     async with primary_session() as session:
#         result = await session.execute(
#             select(WebUsers).where(WebUsers.username == username)
#         )
...
#         result = await session.execute(
#             select(exists().where(GroupToServer.group_id == user.group_id, GroupToServer.server_id == virtual_user_id))
#         ).scalar().first()
